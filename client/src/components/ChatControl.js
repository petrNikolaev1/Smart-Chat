	...
	editForm() {
		...
		return (
			<div className="edit">
				...
				<span className={'edit-info'} onClick={() => this.props.handleMarkedMessage(messageId, true)}> Edit message:
						<span className="message-value">{this.editedTextValidator(editedText)}
						</span></span>
				<span onClick={() => {
					document.getElementById('myTextArea').value = ''
					this.props.handleChatControlContent(id, document.getElementById('myTextArea').value, messageId, false)
				}} className="crossBox">X</span>
			</div>
		)
	}
